...
export function toPredicate(
  expr: Expr,
  targetGetter: (data: any, field: string, expr: Expr) => any = (data, field) => data[field],
): (data: any) => boolean {
  return (data) => {
    if (expr.type === 'ComplexExpr') {
      switch (expr.operator) {
        case 'AND':
          return expr.operands.every((op) =>
            toPredicate(op, targetGetter)(data),
          );
        case 'OR':
      ...
      return !toPredicate(expr.operand, targetGetter)(data);
    }
    ...
    switch (expr.type) {
      ...
      case 'TextOp':
        ...
        switch (expr.operator) {
          ...
          case 'contains-words':
            return target.split(/\b/).some((t) => t.toLowerCase() === expr.value.toLowerCase());
          case 'matches':
